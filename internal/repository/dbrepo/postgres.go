...
func (m *postgresDBRepo) InsertReservation(res models.Reservation) (int,error){
	...
	err := m.DB.QueryRowContext(ctx,stmt,
			...
			res.StartDate, res.EndDate, res.RoomID, time.Now(),
			time.Now()).Scan(&newID)
...
func (m *postgresDBRepo) InsertRoomRestriction(r models.RoomRestriction) error {
	...
	_, err := m.DB.ExecContext(ctx,stmt,r.StartDate,r.EndDate,r.RoomID,r.ReservationID,
			time.Now(),time.Now(), r.RestrictionID)
...
func (m *postgresDBRepo) GetUserByID(id int) (models.User, error){
	...
	row, _ := m.DB.QueryContext(ctx,query,id)
	err := row.Scan(&user.ID,&user.FisrtName,&user.LastName,&user.Email,&user.Password,&user.AccessLevel,
...
func (m *postgresDBRepo) UpdateUser(u models.User) (error) {
	...
	query := `
		update users set first_name = $1 , last_name = $2 , email = $3 , access_level = $4 , updated = $5
		where id = $6
	`
